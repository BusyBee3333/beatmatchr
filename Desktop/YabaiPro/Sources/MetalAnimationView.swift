...
struct MetalView: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> MTKView {
        ...
        let device = MetalAnimationEngine.shared.metalDevice
        print("🎯 MetalView device: \(device != nil ? "✅ Available" : "❌ Nil")")
    ...
    class Coordinator: NSObject, MTKViewDelegate {
        ...
        func draw(in view: MTKView) {
            print("🎨 MetalAnimationView draw called for animation: \(animationType)")
            guard let drawable = view.currentDrawable else {
                print("❌ MetalAnimationView: No drawable available")
                return
            }

            // Check if Metal is ready
            guard engine.isMetalAvailable else {
                print("❌ MetalAnimationView: Metal not available")
                return
            }

            guard engine.areShadersCompiled else {
                print("❌ MetalAnimationView: Shaders not compiled")
                // Clear to a visible color to indicate shader failure
                let renderPass = MTLRenderPassDescriptor()
                renderPass.colorAttachments[0].texture = drawable.texture
                renderPass.colorAttachments[0].loadAction = .clear
                renderPass.colorAttachments[0].clearColor = MTLClearColor(red: 1.0, green: 1.0, blue: 0.0, alpha: 1.0) // Yellow background
                renderPass.colorAttachments[0].storeAction = .store

                if let commandBuffer = engine.metalDevice?.makeCommandQueue()?.makeCommandBuffer(),
